<template>
<uni-shadow-root class="vant-weapp-dist-search-index"><view :class="(utils.bem('search', { withaction: showAction || useActionSlot }))+' custom-class'" :style="'background: '+(background)">
  ...
  <view v-if="showAction || useActionSlot" class="van-search__action" hover-class="van-search__action--hover" hover-stay-time="70">
    ...
    <view v-else @click="onCancel" class="van-search__action-button cancel-class">{{ actionText }}</view>
  </view>
...
global['__wxRoute'] = 'vant-weapp/dist/search/index'
...
export default global['__wxComponents']['vant-weapp/dist/search/index']
</script>
